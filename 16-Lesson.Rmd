...
On a distance function plot when you see a separation between the black solid line and the red line, you should ask "Is this separation large relative to sampling variation?" Said another way. Is the  difference between the empirical and theoretical distance functions large enough to conclude there is significant clustering?
...
Compute the nearest neighbor function ($G(r)$) using the `Gest()` function with a Kaplan-Meier correction for the borders (i.e., `correction = "km"`). Make a plot of this function that also includes a theoretical curve under the null hypothesis of complete spatial randomness. From the plot eyeball an estimate of the percentage of tornadoes there are within 15 km of another tornado.
...
The above plots of $G(r)$ and $K$ show a difference in the spatial distribution of the tornadoes when compared with the spatial distribution of events generated from the null hypothesis model of CSR.
...
Here you then convert the output to a data frame. It takes a few seconds to complete the computation of $K$ for all 99 samples.
...
Again the black line is the $K$ function computed on the tornado report locations data and the red line is the same function under CSR. 

The uncertainty ribbon (gray band) connects the point-wise minimum and maximum values of $K$ computed from the 99 generated point pattern samples. 
...
To test the null hypothesis of CSR you need a statistic indicating the departure of $K$ (or any other distance function) computed on the observations from the theoretical $K$.
...
Here you use the test on the $G(r)$ function with the `fun = Gest` argument and 999 simulations.
...
The value is relative large on the scale from 0 to 1 so the $p$-value is small and you reject the null hypothesis of CSR for these data. This is consistent with the evidence gleaned from the graph. Note: Since there are 999 simulations the lowest $p$-value is .001.
...
Again the $p$-value on the test statistic against the two-sided alternative is less than .01 so you reject the null hypothesis of CSR.
...
The other approach to inference the procedure uses re-sampling. Re-sampling refers to generating samples from the data while sampling refers to generating samples from some theoretical model.
...
Analogues to the $G(r)$ and $K$ functions are available for 'multi-type' point patterns where the marks are factors.
...
The spatial scale matters. Departures from CSR might depend on the lag distance so it is important to be able to interpolate values output from the various distance function.
