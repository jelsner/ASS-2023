...
- Working with {geoR} package spatial data
...
As an example, you compute and plot the sample variogram from the `s100` data set from the {geoR} package and label the key features. The `variog()` function computes the sample variogram values and we create a data frame from the resulting variogram object.

```{r}
install.packages(here::here("packages", "RandomFields_3.3.1.tar.gz"), 
                 repos = NULL, type="source")
install.packages(here::here("packages", "geoR_1.8-1.tar.gz"), 
                 repos = NULL, type="source")
```
...
The {geoR} package contains functions for spatial interpolation. There are other packages that have functions for spatial interpolation, but {geoR} was one of the first comprehensive packages devoted to it. The functions are helpful in learning the required steps. The functions require the data to be of class `geodata`.
