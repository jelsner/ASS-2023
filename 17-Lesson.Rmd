...
## Fitting a log-Gaussian Cox process model to one-dimensional point pattern data

<https://inlabru-org.github.io/inlabru/articles/web/1d_lgcp.html>

Using the method of stochastic partial differential equations.
...
The argument `prior.range =` accepts a vector of length two with the first element the lag distance (range) of the spatial autocorrelation and the second element the probability that the range will be less than that value. If the second value is `NA`, then the value of the first element is used as a fixed range.

The argument `prior.sigma =` accepts a vector of length two with the first element is the marginal standard deviation of the field and the second element the probability that the standard deviation will be greater than that value.
...
Here you want to fit the model to the actual points. The {inlabru} function that does this is `lgcp()` (for 'log Gaussian Cox process'). The `domain =` argument specifies the 1D mesh as a list object.
...
You can look at the posterior distributions of the model parameters using the function `spde.posterior()`. It returns x and y values for a plot of the posterior probability density function (PDF) in a data frame, which can be plotted using using the `plot.bru()` function. 
...
Here is the PDF for the Matern correlation
