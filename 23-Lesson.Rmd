...
Here you consider observations of heavy metal concentrations in the flood plain of the Meuse River in Holland. The data are available in {sp} package.
```{r}
library(sp)
...
Interest is on the spatial distribution of the heavy metals in the soils.

Map the zinc levels at the observation locations.

```{r}
tmap::tmap_mode("view")
tmap::tm_shape(meuse.sf) +
  tmap::tm_dots(col = "zinc")
```

The soil samples are taken in the flood plain of the Meuse river outside the city of Stein. Highest concentrations of zinc are found nearest to the river.


First you organize the data as a `gstat` object. This is done with the `gstat()` function which orders (and copies) the field variables into a single object. It is done successively with the `gstat()` function.
...
On the other hand, if you want to spatially interpolate rare or extreme events then kriging 

Kriging works does on data that can be adequately described with a normal distribution. tends to smooth out local details of the spatial variation of the attribute. Typically, small values are overestimated, whereas large values are underestimated. Such conditional bias is a shortcoming when trying to detect patterns of extreme values. A
