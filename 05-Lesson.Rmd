...
## Working with spatial data {.unnumbered}
...
R's spatial ecosystem continues to evolve. Most changes build on what has been done. Occasionally there is a significant change that builds from scratch. The introduction of the {sf} package in about 2018 (Edzer Pebesma) is a significant change.
...
Usually you don't need to create geometries. Geometries are typically part of the spatial data you import. However, there are a set of functions to create simple feature geometry objects (`sfg`) from scratch, if needed. The names of these functions are simple and consistent, as they all start with the `st_` prefix and end with the name of the geometry type in lowercase letters:
