...
Make a map showing the boundaries of Florida and the boundaries of Bay County.
...
Additional spatial data are added to the map object with `+ tm_shape(new_object)`, where `new_object` is another spatial data frame the values of which are used to plot on top of the preceding layers.
...
Putting two or more maps with the same scale side by side allows for comparisons and to see how spatial relationships change with respect to another variable. Creating small multiples of the same map with different variables is called 'faceting'.
...
You can change the underlying layers byto OpenStreetMap by clicking on the layering symbol.

With the interactive mode turned on, all maps produced with {tmap} launch as zoom-able HTML. This feature includes the ability to specify the base map with `tmap::tm_basemap()` (or `tmap::tmap_options()`) as demonstrated here.
