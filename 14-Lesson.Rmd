# Thursday March 9, 2023 {.unnumbered}
...
Typical work flow includes importing and munging data as a simple feature data frame and then converting the spatial data frame to a `ppp` object for analysis and modeling. But it is sometimes convenient to do some of the data munging after conversion to a `ppp` object.

## Creating `ppp` and `owin` objects from simple feature data frames {.unnumbered}

Consider again the set of Kansas tornadoes. Import the data as a simple feature data frame and transform the geographic CRS to Lambert conic conformal centered on Kansas (EPSG:6922). Keep all tornadoes (having an EF damage rating) since 1950 whose initial location occurred within the state.
...
There are 4281 events (tornado reports) with an average spatial intensity of .0000000201 (2.008277e-08) events per unit area.
...
Caution here as you are recycling the object name `T.ppp`. If you run the above code chunk again the scale will change again by a factor of 1000 while the unit name will stay the same.
...
## Covariate trend as a possible confounding factor {.unnumbered}
...
Additional resource: <https://r-spatial.org/book/11-PointPattern.html>
